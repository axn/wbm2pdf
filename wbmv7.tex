...
Figure \ref{fig:netperf} shows the TCP troughput, measured successively
for each protocol and round...

\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\textwidth]{\selection/netperf_TP.pdf}
\caption{TCP troughput, measured successively for each protocol and round}
\label{fig:netperf}
\end{figure}
...
no means representive. Looking at all perfromed measurements, as given
...
always notable low.  Regarding protcol-traffic overhead, it typically
ranged within the most expensive (bandwidth consuming) protocls.
...
protocol.  Regarding protcol-traffic overhead, it showed average cost
...
average cost.  Regarding protcol-traffic overhead, it always showed
least cost.
...
usage could not be measured. Regarding protcol-traffic overhead, it
